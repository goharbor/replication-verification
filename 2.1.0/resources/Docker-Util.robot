...
Push image
    [Arguments]  ${ip}  ${user}  ${pwd}  ${project}  ${image}  ${need_pull_first}=${true}  ${sha256}=${null}  ${is_robot}=${false}  ${tag_suffix}=${false}
    ...
    ${image_with_sha256}=   Set Variable If  ${need_pull_first}==${true}  ${image_with_sha256}  ${image}
    Run Keyword If  ${need_pull_first}==${true}   Docker Pull  ${LOCAL_REGISTRY}/${LOCAL_REGISTRY_NAMESPACE}/${image_with_sha256}
    ${d}=    Get Current Date    result_format=%m%s
    ${image_with_tag}=  Set Variable If  '${tag_suffix}'=='${true}'  ${image_with_tag}_${d}  ${image_with_tag}
    Run Keyword If  ${is_robot}==${false}  Wait Unitl Command Success  docker login -u ${user} -p ${pwd} ${ip}
    ...  ELSE  Wait Unitl Command Success  docker login -u robot\\\$${user} -p ${pwd} ${ip}
    Run Keyword If  ${need_pull_first}==${true}  Wait Unitl Command Success  docker tag ${LOCAL_REGISTRY}/${LOCAL_REGISTRY_NAMESPACE}/${image_with_sha256} ${ip}/${project}/${image_with_tag}
    ...  ELSE  Wait Unitl Command Success  docker tag ${image_with_sha256} ${ip}/${project}/${image_with_tag}
